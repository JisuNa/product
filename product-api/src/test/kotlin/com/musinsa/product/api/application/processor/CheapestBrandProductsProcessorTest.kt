...
val cheapestBrandProductsProcessor = CheapestBrandProductsProcessor(productRepository)
...
class CheapestBrandProductsProcessorTest : BehaviorSpec({
    given("단일 브랜드 최저가 조회가 요청된 상황에서") {
        `when`("최저가 브랜드가 존재하는 경우") {
            ...
            every { productRepository.findProductsByBrandId(any()) } returns listOf(productDto)
